...
courses_tm_fmla_staff <- staffing_courses %>%
  ...
  mutate(missing_begining_state_date = if_else(work_start_date > ymd("2023-09-01"), ymd("2023-09-01"), NA),
         missing_begining_end_date = if_else(!is.na(missing_begining_state_date),  work_start_date - days(1), NA)) %>%
  mutate(missing_begining_days_out = as.numeric(difftime(ymd(missing_begining_end_date), 
                                        ymd(missing_begining_state_date), units = "days"))) %>%
  mutate(missing_ending_state_date = if_else(work_end_date < ymd("2024-05-01"),work_end_date + days(1), NA),
         missing_ending_end_date = if_else(!is.na(missing_ending_state_date), ymd("2024-05-27"), NA)) %>%
  mutate(missing_ending_days_out = as.numeric(difftime(ymd(missing_ending_end_date), 
                                                         ymd(missing_ending_state_date), units = "days"))) %>%
