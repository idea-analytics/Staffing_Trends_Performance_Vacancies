...
# distinct_campus_name <- jobvite_staffing_vacancies %>%
#   select(campus_upper) %>%
#   distinct()
# distinct_campus_name
# 
# distinct_internal_role <- jobvite_staffing_vacancies %>%
# select(internal_role) %>%
#   distinct(internal_role) 
# distinct_internal_role
# 
# distinct_course_credit_type <- courses_tm_fmla_staff %>%
#   select(course_credit_type) %>%
#   distinct()
# distinct_course_credit_type
...
#look at data
#vacancys, filtered for all TX regions
#look at pmpc_comment1 for teacher (or not teacher) and area (for teacher/instructional, or other)

IDEA_teacher <-jobvite_staffing_vacancies %>%
  ...
  select(employee_id, pmpc_comment1, job_title, area, campus_upper, location.x, region.x, total_days_vacancy)
IDEA_teacher

#distinct pmpc_comment1 and area
distinct_pmpc_comment1 <- IDEA_teacher %>%
  select(pmpc_comment1) %>%
  distinct()
distinct_pmpc_comment1

#distinct area
distinct_area <- IDEA_teacher %>%
  select(area) %>%
  distinct()
distinct_area
...
# SCHOOL LEAD TEAM       #not instructional/teacher

#check to see how many rows for area not teacher, pmpc not teacher
IDEA_teacher_area <- IDEA_teacher %>%
  filter(area == "SCHOOL LEAD TEAM")
IDEA_teacher_area         #12 rows

#check campus_upper, look for HQ
distinct_campus <- IDEA_teacher %>%
  select(campus_upper) %>%
  distinct()
distinct_campus
...
#check job_title (which will become "subject")
distinct_jobtitle <- IDEA_teacher %>%
  select(job_title) %>%
  distinct()
distinct_jobtitle
# ELA
...
# SCIENCE 
# SOCIAL STUDIES
# EXECUTIVE CAMPUS LEADERSHIP   #12 rows, all are also "area" = "school lead team", so should be removed

#-------------------------------------

#Master Data

IDEA_teacher_data <-jobvite_staffing_vacancies %>%
  mutate(Region = region.x) %>%
  filter(Region == "AUSTIN REGION" |
           Region == "EL PASO REGION" |
           Region == "GREATER HOUSTON REGION" |
           ...
           Region == "TARRANT COUNTY REGION") %>%
 # select(employee_id, pmpc_comment1, job_title, area, campus_upper, location.x, region.x, total_days_vacancy) %>%
  filter(job_title != "Executive Campus Leadership") %>%   #12 rows
  filter(area != "SCHOOL LEAD TEAM") %>%  #12 rows (same 12 rows as "executive campus leadership")
  filter(campus_upper != "HQ RGV" |
          campus_upper != "HQ San Antonio"|
          campus_upper != "HQ Tarrant County") 
  IDEA_teacher_data    #714 rows of vacancies
...
#vacancy
IDEA_median_mean_sd <- IDEA_teacher_data %>%
  select(total_days_vacancy) %>%
 #group_by(total_days_vacancy)  %>%
...
#vacancy
IDEA_vacancy_count <-IDEA_teacher_data %>%
  summarise(`Number of Vacancies IDEA Texas` = n()) 
IDEA_vacancy_count
...
#f-1) region - VACANCY
region_median_mean_sd <-IDEA_teacher_data %>%
...
region_median_mean_sd_sort <- region_median_mean_sd[order(-Median),]
region_median_mean_sd_sort
